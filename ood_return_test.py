...
Writes:
  reports/ood_return_test.json
  reports/ood_return_test.png
...
def main():
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(REPORTS / "ood_return_test.png", dpi=150)

    (REPORTS / "ood_return_test.json").write_text(json.dumps(results, indent=2))
    print(f"\n[ood_return] wrote {REPORTS/'ood_return_test.json'}")
    print(f"[ood_return] wrote {REPORTS/'ood_return_test.png'}")
